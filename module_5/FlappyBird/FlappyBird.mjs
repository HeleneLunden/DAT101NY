...
export const GameProps = {
  ...
  heroWasAlive: true, //for å vite når flappy går fra levende til død
};
...
export function handleHeroDeath() {
  if (!GameProps.heroWasAlive) return; // Ikke gjør noe om han allerede er "død"

  console.log("handleHeroDeath: Flappy traff bakken – spiller lyd");
  ...
  switch (GameProps.status) {
    case EGameStatus.playing:
      if (GameProps.hero.isDead) {
        ...
        if (GameProps.heroWasAlive) {
          console.log("Flappy was alive – nå død for første gang");
      
          const heroBottom = GameProps.hero.getBottom();
          const groundTop = GameProps.ground.getTop();
      
          console.log("🔍 Flappy getBottom:", heroBottom, "Ground getTop:", groundTop);
      
          // Spill lyden uansett hva vi tror om posisjon – TEST
          console.log("🧪 TEST: Spiller heroIsDead-lyd uansett");
          GameProps.sounds.heroIsDead.pause();
          GameProps.sounds.heroIsDead.currentTime = 0;
          playSound(GameProps.sounds.heroIsDead);
    ...
    case EGameStatus.gameOver:
      ...
      if (delBaitIndex >= 0) {
        ...
        //Spill poeng-lyd riktig hver gang
...
function spawnObstacle() {
  ...
  // Sett riktig sprite basert på dag/natt
  obstacle.index = GameProps.dayTime ? 2 : 0;

  GameProps.obstacles.push(obstacle);

  // Spawn ny hindring etter 2–7 sekunder
...
function setDayNight() {
  if (rbDayNight[0].checked) {
    ...
    console.log("Day mode valgt");
    GameProps.background.index = 0; // Lys bakgrunn
  } else {
    GameProps.dayTime = false;
    console.log("Night mode valgt");
    GameProps.background.index = 1; // Mørk bakgrunn
  }

  // Sett hindringene til riktig index (2 for grønt/dag, 0 for rødt/natt)
  for (let i = 0; i < GameProps.obstacles.length; i++) {
    GameProps.obstacles[i].index = GameProps.dayTime ? 2 : 0;
  }
}
  ...
  switch (aEvent.code) {
    case "Space":
      if (GameProps.status === EGameStatus.idle) {
        startCountdownAndGame(); // hvis du har nedtelling
      } else if (!GameProps.hero.isDead) {
        GameProps.hero.flap();
